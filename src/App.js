...
const InterestForm = () => {
  const [form, setForm] = useState({ name: '', mobile: '', email: '', model: '' });
  ...
  return (
    <>
      <nav className='fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 md:px-12 lg:px-16 py-3 bg-white/30 backdrop-blur-lg shadow-md'>
        <img src='https://bharath-hyundai-2-0-git-main-bharath-hyundais-projects.vercel.app/Hyundai-logo.png' className='h-12' alt='Hyundai Logo' />
      ...
      <div className='relative w-full min-h-screen mt-14 md:mt-10 hidden md:block'>
        <img src='https://bharath-hyundai-2-0-git-main-bharath-hyundais-projects.vercel.app/Banner.webp' alt='Banner' className='absolute inset-0 w-full h-full object-cover' />
      </div>
      ...
      <div className='w-full bg-white flex justify-center border md:top-2/3 lg:absolute lg:top-1/2 lg:left-72 lg:transform lg:-translate-x-1/2 lg:-translate-y-1/2 lg:bg-white lg:p-6 lg:rounded-2xl lg:shadow-lg lg:w-[90%] lg:max-w-lg'>
        <div className='bg-white p-6 sm:p-8 md:p-10 rounded-2xl shadow-lg w-full max-w-md lg:max-w-lg'>
          <h3 className='text-xl sm:text-2xl font-bold text-black text-center pb-4'>REGISTER YOUR INTEREST</h3>
          {!submitted && (
            <form onSubmit={handleSubmit} className='space-y-6'>
              <input type='text' name='name' placeholder='Name' value={form.name} onChange={handleChange} className='w-full px-4 py-2 border-b-2 border-black bg-transparent text-black text-center text-sm sm:text-base focus:outline-none' />
              {errors.name && <p className='text-red-500 text-xs'>{errors.name}</p>}
              <input type='text' name='mobile' placeholder='Mobile Number' value={form.mobile} onChange={handleChange} className='w-full px-4 py-2 border-b-2 border-black bg-transparent text-black text-center text-sm sm:text-base focus:outline-none' />
              {errors.mobile && <p className='text-red-500 text-xs'>{errors.mobile}</p>}
              <input type='email' name='email' placeholder='Email (optional)' value={form.email} onChange={handleChange} className='w-full px-4 py-2 border-b-2 border-black bg-transparent text-black text-center text-sm sm:text-base focus:outline-none' />
              <select name='model' value={form.model} onChange={handleChange} className='w-full px-4 py-2 border-b-2 border-black bg-transparent text-black text-center text-sm sm:text-base focus:outline-none'>
              ...
              <button type='submit' className='w-full bg-blue-600 text-white py-3 rounded-lg font-bold text-sm sm:text-base hover:bg-blue-700 transition duration-200'>Submit</button>
            </form>
          )}
          <p className='text-xs text-gray-600 mt-3 text-center'>*By clicking 'Submit', you agree to our Terms & Conditions.</p>
        </div>
      </div>
      <OffersCarousel/>
      <CarShowcase/>
  ...
  return (
    <footer className="bg-gray-900 text-white py-10">
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div>
          ...
          <p className="mt-2 flex items-start gap-2">
            ...
            <span>#3,4,5,6 Survey No 58/1, Gowliddodi, Gopanpally, Financial District, Gachibowli-500075</span>
          </p>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {["Hyundai Creta", "Hyundai Creta EV", "Hyundai i20", "Hyundai Verna", "Hyundai Venue"].map((car, index) => (
              <li key={index} className="border-b pb-1">{car}</li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {["About Bharat Hyundai Motors", "Gallery", "Testimonials", "Contact Us", "Career"].map((item, index) => (
              <li key={index} className="border-b pb-1">{item}</li>
            ))}
          </ul>
        </div>
      ...
      <div className="border-t border-gray-700 mt-6 pt-4 text-center text-sm">
        ...
        <p className="mt-2">© 2025 All Rights Reserved by Bharat Hyundai Motors.</p>
        <p className="mt-1 text-gray-500">
          Powered by{" "}
          <a href="https://broaddcast.com/" target="_blank" rel="noopener noreferrer">
            <span className="hover:text-red-500">BroaddCast Business Solutions LLP.</span>
          </a>
        </p>
      </div>
    </footer>
